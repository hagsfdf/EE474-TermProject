...
from net import CNN_model, CNN_model_brand


dir_data = "./imagebrand/"

data = np.load(dir_data + "train.npy")
filenames = data['dir']
label1 = data['price']
label2 = data['brand']
N = 512  #len(label1) # 1284
n_class = 4
n_class_brand = 5

def brand_loading():
    one_hot_brand = np.zeros([N, n_class_brand], dtype = np.float32)
    for i in range(N):
        label = label2[i]
        brand = ['adidas','descente','fila','nike','puma']
        one_hot_brand[i,:] = np.isin(brand,label)
    print(np.sum(one_hot_brand, axis=0))
    return one_hot_brand
...
def main():
    ...
    ## 차원 알려주세욤
    input_brand = tf.placeholder(tf.float32, shape = [], name = 'brand')
    ## need shape
